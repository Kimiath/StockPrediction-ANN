...
# Deeveloping feature generation function (feature engineering)
...
data_raw = pd.read_csv('./', index_col='Date')
...
#Fine Tuning the model by adding more hidden layers in the neural network
hidden_size = 16
epochs = 3000
lr = 0.3
best_model = nn.Sequential(nn.Linear(X_train.shape[1], 16),
                           nn.ReLU(),
                           nn.Linear(16, 1))
optimizer = torch.optim.Adam(best_model.parameters(), lr=lr)
for epoch in range(epochs):
...
#Applying dropout/early stopping technique to see if the R2 increases(Current R2 score is 0.977)
# Dropout
...
# optimizer = torch.optim.Adam(model_with_dropout.parameters(), lr=0.3)
